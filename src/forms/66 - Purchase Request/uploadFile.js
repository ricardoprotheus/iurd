	...
	$.ajax({
         ...
         success:function(model) {
        	...
        	console.log(model.content.id);
       		console.log(model);
        	$("#hd_attachment").val(model.content.id);
			$("#txt_fileID").val(model.content.id);
			$("#txt_fileName").val(model.content.description);
		}
	});
	
}

function viewDocument(idCampo){
	var documentId = $("#"+idCampo).val(); 
	if(documentId != ""){
